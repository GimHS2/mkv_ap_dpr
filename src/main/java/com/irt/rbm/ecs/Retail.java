 ...
 *
 */
public class Retail extends com.irt.rbm.ManipulableManagerImpl {
	private final static Queryable queryable = new QueryableImpl( Schema.findQueryable(Schema.ECS_RETAIL) ) {{
		append( Schema.findQueryable(Schema.ECS_PARTY), "PTY.GLN = RTL.RT_GLN" );
	}};

	private final static Queryable queryable_pty = new QueryableImpl( Schema.findQueryable(Schema.ECS_PARTY) ) {
		{
			append( Schema.findQueryable(Schema.ECS_RETAIL), "RTL.RT_GLN(+) = PTY.GLN" );
		}

		@Override
		public boolean appendTable( QueryBuffer querybuf ) {
			if( super.appendTable(querybuf) ) {
				querybuf.appendCondition( "PTY.PARTY_ROLE = 'RTL'" );
				return true;
			}
			return false;
		}
	...
	private final static Table table = Schema.findTable( Schema.ECS_RETAIL );
	...
	public Retail( SQLHandler handler ) {
		super( handler, table, factory );
	}

	public static Map<String, Object> createPrimary( String retailGln ) {
		return Record.createMap( "retailGln", retailGln );
	}

	@Override
	public int getRecordCount( Map<String, ? extends Object> conditionMap ) throws SQLException {
		QueryFactory factory = ( Condition.isConditionTrue(conditionMap, "analysisOnly") ? Retail.factory : Retail.factory_pty );
	...
	public boolean regist( Map<String, Object> recordMap ) throws DataException, SQLException {
		SQLManager.executeStatement( handler, "INSERT INTO RBM_MNG_KEY( MNGKEY ) VALUES( 'R' || seqRBM_MNG_KEY.NEXTVAL )" );
