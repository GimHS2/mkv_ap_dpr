 ...
 *	jbaek		2017/09/30		2.2.8c	FlexListWriter class to be added to ListWriter class
...
import com.irt.html.HyperLink;
import com.irt.html.LinkedColumn;
...
/**
 * logs data cell length
 * @deprecated use ListWriter.setUseFlexDataLine.
 */
//public class FlexListWriter implements com.irt.html.ListWriter {
@Deprecated
public class FlexListWriter implements com.irt.html.ListWriter, PageListIdentifiable {
	...
	public FlexListWriter( HttpServletRequest request, HtmlPage htmlpage ) {
		this(request, htmlpage, (ColumnList)request.getAttribute("columnList"));
	}

	public FlexListWriter( HttpServletRequest request, HtmlPage htmlpage, String listKey ) throws IllegalArgumentException {
		this(request, htmlpage, (ColumnList)request.getAttribute("columnList"), listKey);
	}

	public FlexListWriter( HttpServletRequest request, HtmlPage htmlpage, ColumnList columnList ) throws IllegalArgumentException {
		this(request, htmlpage, columnList, (String)null);
	}

	public FlexListWriter( HttpServletRequest request, HtmlPage htmlpage, ColumnList columnList, String listKey )
		...
		this.dataMaxWidths = new int[columns.length];
	}

	public FlexListWriter( HttpServletRequest request, HtmlPage htmlpage, ColumnList columnList, ColumnList summaryColumnList ) {
		this(request, htmlpage, columnList);
		setSummaryColumnList(summaryColumnList);
	}

	public FlexListWriter( HttpServletRequest request, HtmlPage htmlpage, ColumnList columnList, ColumnList[] summaryColumnLists ) {
		this(request, htmlpage, columnList);
	...
	public String getDataCellAttr( Column column, int row ) {
		...
		if( dataCellAttr != null )
			columnAttr = columnAttr + " " + dataCellAttr;
	...
	public void print( JspWriter out, int rowsize ) throws IOException {
		...
		if( useVerticalScroll && this.heightResizable && printingHeader
			...
			out.println(
					"<div class='list_content_header_scroll' style='height: expression(this.childNodes(0).offsetHeight+2); overflow-y: scroll;'>");
		...
		if( printingErrors )
			printErrors(out, ( rowsize * 2 - 1 ));

		if( records == null || records.size() == 0 ) {
			out.println(
					"<tr><td colspan='" + ( rowsize * 2 - 1 ) + "' class='list_msg'>" + msghandler.getMessage("MSG_NO_RECORD_FOUND") + "</td></tr>");
			out.println("</table>");
		} else {
			if( !printingHeader )
				out.println("<tr><td colspan='" + ( rowsize * 2 - 1 ) + "'/></tr>");
			out.println("</table>");
	...
	private void setMaxWidth( int col, int currLen ) {
		if( dataMaxWidths[col] < currLen )
			dataMaxWidths[col] = currLen;
	}
	...
	public void printDataCell( JspWriter out, Column column, Map recordMap, int row, int col ) throws IOException {
		...
		setMaxWidth(col, columnValue.length());
		if( hyperLink != null ) {
	...
	public void printDataLine( JspWriter out, Map recordMap, int row ) throws IOException {
		...
		if( checkboxType == CHECKBOXTYPE_CHECK || checkboxType == CHECKBOXTYPE_RADIO ) {
			if( checkboxType == CHECKBOXTYPE_CHECK )
				out.print("<td class='check'><input type='checkbox' name='" + checkboxName + "'");
			else
				out.print("<td class='check'><input type='radio' name='" + checkboxName + "'");
			out.print(" id='" + checkboxName + "_" + row + "' onClick='JavaScript:listClick(this);'");

			String checkboxValue = getCheckBoxValue(recordMap);
			if( checkboxValue != null )
				out.print(" value='" + checkboxValue + "'");
			if( !enableCheckbox(recordMap, row) )
		...
		if( numbering )
			out.print("<td class='number'>" + ( startingNumber + row + 1 ) + "</td>");
	...
	public void printDataLineAttribute( JspWriter out, Map recordMap, int row ) throws IOException {
		out.print(" class='list_content' onMouseOut='JavaScript:listMouseOut(this);' onMouseOver='JavaScript:listMouseOver(this);'");
	...
	public void printHead( JspWriter out ) throws IOException {
		...
		if( useVerticalScroll && this.heightResizable && ( ( records != null && records.size() > 0 ) || ( printingErrors && errors != null ) ) ) {
			if( !printingHeader )
				out.println("<tr><td colspan='" + getColumnSize() + "'/></tr>");
			out.println("</table>");
			if( printingHeader )
				out.println("</div>");
	...
	public void printHeaderCell( JspWriter out, Column column, int rowspan ) throws IOException {
		...
		if( column instanceof LinkedColumn ) {
			...
			if( hyperLink != null && hyperLink.isValidLink(null, sessionMng) ) {
				...
				out.print("<th" + attributeBuffer.toString() + ">");
				out.print("<a href='JavaScript:void(0);' onClick='JavaScript:setEventObject(event);"
		...
		if( column.sortable() && !disableSort ) {
			...
			String imageTag = "<img src='" + imageBasePath + "/ico_sort_none.gif' align='absmiddle'/>";
			if( fieldKey.equals(sortKey) ) {
				if( "DESC".equalsIgnoreCase(sortWay) )
					imageTag = "<img src='" + imageBasePath + "/ico_sort_desc.gif' align='absmiddle'/>";
				else {
					sortScript = "JavaScript:listSort(\"" + fieldKey + "%23DESC\");";
					imageTag = "<img src='" + imageBasePath + "/ico_sort_asc.gif' align='absmiddle'/>";
				}
			}
			if( columnAttr.indexOf("class=") < 0 )
				attributeBuffer.append(" class='header_sort' " + columnAttr + " title='" + msghandler.getMessage("MSG_SORT")
						+ "' onClick='" + sortScript + "'");
			else
				attributeBuffer.append(" class='header_sort " + columnAttr.substring(columnAttr.indexOf("class=") + 7)
	...
	public void printHeaderLine( JspWriter out ) throws IOException {
		...
		String checkScript = "JavaScript:CheckBox.checkAll(" + htmlpage.getFormName() + "." + checkboxName + ")";
		if( hasColumnGroup ) {
			out.print("<tr>");
			if( checkboxType == CHECKBOXTYPE_CHECK )
				out.print("<th rowspan='2'><a href='" + checkScript + "'><img src='" + imageBasePath + "/ico_check.gif'></a></th>");
			else if( checkboxType == CHECKBOXTYPE_RADIO )
				out.print("<th rowspan='2'><img src='" + imageBasePath + "/ico_check.gif'></th>");
			if( numbering )
			...
			for( int c = 0; c < columns.length; c++ ) {
				if( columnGroup == columns[c].getColumnGroup() )
					...
					if( columnGroup != null )
						out.print("<th colspan='" + colspan + "' class='header_top'>" + columnGroup.getGroupTitle(null, msghandler) + "</th>");

					colspan = columns[c].getColumnSize();
					columnGroup = columns[c].getColumnGroup();
				}
				if( columnGroup == null )
					printHeaderCell(out, columns[c], 2);
			}
			if( columnGroup != null )
				out.print("<th colspan='" + colspan + "' class='header_top'>" + columnGroup.getGroupTitle(null, msghandler) + "</th>");
			out.println("</tr>");
			...
			for( int c = 0; c < columns.length; c++ )
				if( columns[c].getColumnGroup() != null )
					printHeaderCell(out, columns[c], 1);
			out.println("</tr>");
		} else {
			out.print("<tr class='header_one'>");
			if( checkboxType == CHECKBOXTYPE_CHECK )
				out.print("<th><a href='" + checkScript + "'><img src='" + imageBasePath + "/ico_check.gif'></a></th>");
			else if( checkboxType == CHECKBOXTYPE_RADIO )
				out.print("<th><img src='" + imageBasePath + "/ico_check.gif'></th>");
			if( numbering )
				out.print("<th>No</th>");
	...
	public void printTableColgroup( JspWriter out, boolean isDataLine ) throws IOException {
		out.print("<colgroup>");
		if( checkboxType == CHECKBOXTYPE_CHECK || checkboxType == CHECKBOXTYPE_RADIO )
			out.print("<col class='check'/>");
		if( numbering )
			out.print("<col class='number'/>");
	...
	private void printFlexDataLine( JspWriter out ) throws IOException {
		...
		for( int c = 0; c < columns.length; c++ ) {
			...
			out.print("<span name='" + columns[c].getFieldKey()
					...
					+ "' data-column-data-max-width='" + dataMaxWidths[c]
					+ "'></span>");
			dataMaxWidthLine += dataMaxWidths[c];
	...
	public void printTail( JspWriter out ) throws IOException {
		if( useVerticalScroll && this.heightResizable && ( ( records != null && records.size() > 0 ) || ( printingErrors && errors != null ) ) ) {
			printFlexDataLine(out);
			out.println("</table>");
			out.println("</div>");

			if( records != null && records.size() > 0 && summaryColumnLists != null ) {
				out.println(
						"<div class='list_content_summary_scroll' style='height: expression(this.childNodes(0).offsetHeight); overflow-y: scroll;'>");
	...
	public void printPageIdentity( JspWriter out ) throws IOException {
		...
		if( summaryColumnLists != null ) {
			for(ColumnList scl : summaryColumnLists) {
				PageMessageKeys.putColumnListName(htmlpage, scl.getName());
			}
		}
