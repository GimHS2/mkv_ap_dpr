 ...
 *	jbaek		2014/12/31		2.2.0	create. 원본에서  isMobileAgent 추가
 *
**/
...
public class UAgentInfo {
	...
	public void initDeviceScan() {
		...
		// 가장 큰 개념으로 desktop과 mobile로 구분시 mobile일 경우 true return. mobile은 타블렛도 포함.
		this.isMobileAgent = detectMobileAgent();
		//mobile중에서 phone만
	}

	/** mobile device ( tablet or phone or any ) 인식
	 * = user-agent가 desktop이 아닌 경우 true. */
	public boolean isMobileAgent = false;
	/** mobile device ( tablet or phone or any ) 인식
	 * = user-agent가 desktop이 아닌 경우 true. */
	public boolean detectMobileAgent() {
		if( detectMobileQuick() || detectTierTablet() || detectTierIphone() )
			return true;
		return false;
	}
