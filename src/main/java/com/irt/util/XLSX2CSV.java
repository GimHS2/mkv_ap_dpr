 ...
 *	jbaek		2013/08/30		2.2.0	copy from poi. xlsx 파일을 csv 파일로 변환.
**/
...
public class XLSX2CSV {
	...
	class MyXSSFSheetHandler extends DefaultHandler {
		...
		public MyXSSFSheetHandler( StylesTable styles, ReadOnlySharedStringsTable strings, int cols, PrintStream target ) {
			...
			this.formatter = new DataFormatter( java.util.Locale.getDefault() );
		}
				...
				switch( nextDataType ) {
					...
					case NUMBER:
						...
						if( this.formatString != null ) {
							...
							if( org.apache.poi.ss.usermodel.DateUtil.isADateFormat(this.formatIndex, this.formatString)
									&& org.apache.poi.ss.usermodel.DateUtil.isValidExcelDate(Double.parseDouble(n)) ) {
								java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat( RBMWorkbook.DEFAULT_DATEPATTERN );
								thisStr = sdf.format( org.apache.poi.ss.usermodel.DateUtil.getJavaDate(Double.parseDouble(n)) );
							}
						} else
							thisStr = n;
						break;
