<%--
	File Name:	dpr_dangerousitem_input.jsp
	Version:	2.2.0
...
<head>
	...
	<script type='text/javascript'>
		function modifyReq() {
			var url = "<%= systemConfig.getClassURL() %>/DPRDangerousItem?mode=imod";
			url = replaceQueryValue( url, "itemCode", "<mtl:value id="record" key="itemCode" encodeScript="true"/>" );
			windowSelfOpen( url );
		}

		function registReq() {
			var url = "<%= systemConfig.getClassURL() %>/DPRDangerousItem?mode=ireg";
			windowSelfOpen( url );
		}
	</script>
</head>
		...
		<mtl:form name="frmMain" fieldSetId="fieldSet" onSubmit="JavaScript:return checkInput();">
			...
			<mtl:contentGroup groupId="main" type="content">
				<div class='info-table'>
					...
					<div class='row'>
						<div class='cell'>
							<div class='field-title'><mtl:title key="manualInd"/></div>
							<div class='field'>
								<mtl:select id="record" key="manualInd" prefixKey="DPR_ITEM_MASTER_PLANT_MANUALIND_" codeValues="Y,N" searchable="false"/>
							</div>
						</div>
					</div>
				</div>
