...
; (function ($, window, document, Util, undefined) {
            ...
            sorting: function () {
                ...
                $('.medium-insert-images').sortable({
                    ...
                    afterMove: function () {
                        ...
                        that.sortingAfterMoveRemovePlaceholders();
                    }
                });
            },
            messages: {
    ...
    Images.prototype.deleteFile = function (file) {
        if (this.options.deleteScript) {
            var attachNumber = getQueryValue( file, 'attachNumber'  );
            var opts = $.extend(this.options.fileDeleteOptions, {attachNumber: attachNumber});
    ...
    Images.prototype.sortingAfterMoveRemovePlaceholders = function () {
        $.proxy(this.$el.find('figure.placeholder').each(function(i,el){
            var $here = $(el);
            var isActive = ($here.parent().hasClass('medium-insert-active'));
            if( ! isActive  ) $here.remove();
        }));
    };
